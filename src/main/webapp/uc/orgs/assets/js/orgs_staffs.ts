    ...
    class StaffSelectController {
        ...
        deleteEntity(row){
            ...
            me.ksTip.confirm("确定将员工【" + row.name + "】从角色【" + me.selectedRole.name + "】中移除?")
                .ok(function(){
                    me.ksEntityService.delete(window.webRoot + '/uc/privileges/roles/delete_users.do',{ids:row.id + "",roleId:me.selectedRole.id})
                        .success(function(){
        ...
        batchRemove(){
            ...
            me.ksTip.confirm("确定移除选中员工?")
                .ok(function(){
                    me.ksEntityService.delete(window.webRoot + '/uc/privileges/roles/delete_users.do',{ids:ids.join(","),roleId:me.selectedRole.id})
                        .success(function(){
                            me.ksTip.success("保存成功");
                            me.selectEntities(true);
                        })
