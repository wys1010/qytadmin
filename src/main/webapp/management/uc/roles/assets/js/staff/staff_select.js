...
(function (RoleIndexApp) {
    ...
    var StaffSelectController = (function () {
        ...
        StaffSelectController.prototype.deleteEntity = function (row) {
            ...
            me.ksTip.confirm("确定将员工【" + row.name + "】从角色【" + me.selectedRole.name + "】中移除?").ok(function () {
                me.ksEntityService.delete(window.webRoot + '/management/uc/privileges/roles/delete_users.do', { ids: row.id + "", roleId: me.selectedRole.id }).success(function () {
        ...
        StaffSelectController.prototype.batchRemove = function () {
            ...
            me.ksTip.confirm("确定移除选中员工?").ok(function () {
                me.ksEntityService.delete(window.webRoot + '/management/uc/privileges/roles/delete_users.do', { ids: ids.join(","), roleId: me.selectedRole.id }).success(function () {
                    me.ksTip.success("保存成功");
                    me.selectEntities(true);
                }).error(function () {
                    me.ksTip.error("保存失败");
    ...
    var app = angular.module('RoleIndexApp').controller("StaffSelectController", StaffSelectController);
